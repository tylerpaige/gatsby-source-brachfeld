...
const prepareJSONNode = (json, ctx) => {
  ...
  const JSONNode = { ...(_lodash.default.isPlainObject(json) ? { ...json
    } : {
    ...
    children: [],
    internal: {
      type: _lodash.default.toUpper(`${parentNode.internal.type}_${attributeName}_JSONNode`),
...
const prepareRelationNode = (relation, ctx) => {
  const {
    ...
    parentNode,
    targetSchemaUid
  } = ctx; // const targetSchema = getContentTypeSchema(schemas, targetSchemaUid);
  // const {
  //   schema: { singularName },
  // } = targetSchema;
...
const prepareTextNode = (text, ctx) => {
  ...
  const textNode = {
    ...
    [attributeName]: text,
    internal: {
      type: _lodash.default.toUpper(`${parentNode.internal.type}_${attributeName}_TextNode`),
...
const createNodes = (entity, ctx, uid) => {
  const nodes = [];
  const {
    schemas,
    createNodeId,
  ...
  for (const attributeName of Object.keys(entity)) {
    ...
    const type = _lodash.default.get(attribute, 'type', null);
    ...
    if (value) {
      ...
      if (type === 'dynamiczone') {
        value.forEach(v => {
          ...
          const valueNodes = _lodash.default.flatten(createNodes(v, ctx, v.strapi_component));
      ...
      if (type === 'component') {
        ...
        if (attribute.repeatable) {
          const compoNodes = _lodash.default.flatten(value.map(v => createNodes(v, ctx, attribute.component))); // Just link the component nodes and not the components' children one
          ...
          const compoNodes = _lodash.default.flatten(createNodes(value, ctx, attribute.component)); // Just link the component node and not the component's children one


          entity[`${attributeName}___NODE`] = compoNodes.filter(({
            internal
          }) => internal.type === componentNodeName)[0].id;
          compoNodes.forEach(node => {
            nodes.push(node);
          });
        }
      ...
      if (type === 'json') {
        ...
        entity[`${attributeName}___NODE`] = JSONNode.id; // Resolve only the attributeName___NODE and not to both ones
